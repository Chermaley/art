...
/* harmony import */ var _modules_img__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./modules/img */ "./src/js/modules/img.js");
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_img__WEBPACK_IMPORTED_MODULE_8__["default"])('.sizes-block');
...
var accordion = function accordion(triggerSelector) {
  var heading = document.querySelectorAll(triggerSelector);
  heading.forEach(function (btn) {
...
var calc = function calc(size, material, options, promocode, result, state) {
  ...
  var changeModalState = function changeModalState(element, prop) {
...
var drop = function drop() {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      var dots; //Обрезаем имя изображения если оно больше 6 символов
    ...
    if (target && target.tagName == 'LI') {
      ...
      markAll.forEach(function (item) {
        if (!target.classList.contains('active')) {
          //Если таб уже выбран, то контент не скрываем!
          item.style.display = 'none';
          item.classList.remove('animated', 'fadeIn');
        }
      });
      var classBtn = target.classList.value; //открываем таб с соответствующим классом
...
var forms = function forms(state) {
  //Селектор последнего окна пекредаем, чтобы закрыть после отправки
  var form = document.querySelectorAll('form'),
      inputs = document.querySelectorAll('input'),
      //берем все инпуты, чтобы потом их очистить
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      //event нужен для того, чтобы страница не пперезагружалась
      e.preventDefault(); //Создаем блок для оповаещения и скрываем форму
      ...
      setTimeout(function () {
        item.style.display = "none";
      }, 400); //Добавляем в этот блок картинку и текстовое оповещение
      ...
      var api; //Отправляем форму на определнный сервер в зависимомти от класса
...
var img = function img(imgSelector) {
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {// elem.addEventListener('click', ()=>{
...
var modal = function modal() {
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      ...
      var scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); //Проверяем долистал ли пользователь до конца
  ...
  function openModalTimer(selector, time) {
    setTimeout(function () {
      var display; //Проверка на то что модальное окно уже не открыто
  ...
  openByScroll('.fixed-gift'); // openModalTimer('.popup-consultation', 60000);
};
...
var scrolling = function scrolling(upSelector) {
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      var body = document.body,
          widthTop = document.documentElement.scrollTop || Math.round(body.scrollTop),
          hash = this.hash,
  ...
  // const element = document.documentElement, //Две переменные для адаптации 
  //       body = document.body;
  // const calcScroll = () => {
...
var slider = function slider(slides, dir, prev, next) {
  var slideIndex = 1,
      paused = false;
  var items = document.querySelectorAll(slides); //   slider = document.querySelector(sliderSelector);
      ...
      switch (_context.prev = _context.next) {
        case 0:
          ...
          return regeneratorRuntime.awrap(fetch(url, {
            //Ставим await чтобы дождаться завершения отправки
            method: "POST",
            body: data
          }));
